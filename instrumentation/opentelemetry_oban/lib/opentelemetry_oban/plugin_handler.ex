defmodule OpentelemetryOban.PluginHandler do
  ...
  def attach(opts) do
    attach_plugin_start_handler(opts)
    attach_plugin_stop_handler(opts)
    attach_plugin_exception_handler(opts)
  end

  defp attach_plugin_start_handler(opts) do
    :telemetry.attach(
  ...
  defp attach_plugin_stop_handler(opts) do
    :telemetry.attach(
  ...
  defp attach_plugin_exception_handler(opts) do
    :telemetry.attach(
  ...
  def handle_plugin_start(_event, _measurements, %{plugin: plugin} = metadata, sampler: sampler) do
